...
class InceptionI3d(nn.Module):
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True,
        ...
        self.avg_pool = nn.AvgPool3d(kernel_size=[2, 7, 7],
                                     stride=(1, 1, 1))
        self.avg_pool_1 = nn.AvgPool3d(kernel_size=[1, 7, 7],
                                     stride=(1, 1, 1))
    ...
    def forward(self, x, pretrained=False, n_tune_layers=-1):
        ...
        return self.avg_pool_1(x)
        # mixed_5c = self.avg_pool(x)
        # # head
        # x = self.logits(self.dropout(self.avg_pool(x)))
        # if self._spatial_squeeze:
        #     logits = x.squeeze(3).squeeze(3)
        # # logits is batch X time X classes, which is what we want to work with
        # return logits, mixed_5c
        # # print(mixed_5c.shape)
        # # return mixed_5c, mixed_5c
