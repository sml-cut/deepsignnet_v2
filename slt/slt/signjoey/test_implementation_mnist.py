...
Currently used architecture: LeNet-300-100
...
class Net(nn.Module):
    ...
    def __init__(self):
        """
        Here we define the layers we are going to use.
        """

        super(Net, self).__init__()
        self.ibp = False
        self.fc1 = DenseBayesian(input_features=784, output_features=300, competitors = 1, activation = 'lwta',
                   prior_mean=0, prior_scale=1. , ibp = self.ibp)
        self.fc2 = DenseBayesian(input_features=300, output_features=100, competitors = 1, activation = 'lwta',
                   prior_mean=0, prior_scale=1. , ibp = self.ibp)
        self.fc3 = DenseBayesian(input_features=100, output_features=10, competitors = 1, activation = 'linear',
                   prior_mean=0, prior_scale=1. , ibp = self.ibp)
...
def test(model, device, test_loader, train_loader):
    ...
    :param model: nn.Module: instance implementing the defined model
    :param device: str: the device to use
    :param test_loader: torch DataLoader: loader for the test data
    :param train_loader: torch DataLoader: loader for the train data
    ...
    if model.ibp:
        ...
        shape = list(model.children())[0].t_pi.size(0)*list(model.children())[0].t_pi.size(1)
        print('Layer 1:', (torch.sigmoid(list(model.children())[0].t_pi)<1e-2).sum(),'/',shape)

        shape = list(model.children())[1].t_pi.size(0) * list(model.children())[1].t_pi.size(1)
        print('Layer 2:', (torch.sigmoid(list(model.children())[1].t_pi)<1e-2).sum(),'/',shape)

        shape = list(model.children())[2].t_pi.size(0) * list(model.children())[2].t_pi.size(1)
        print('Layer 3:', (torch.sigmoid(list(model.children())[2].t_pi)<1e-2).sum(),'/',shape)
        print('########################\n')
...
def main():
    """
    Main function: define the args, load dataset and construct train and test stuff.
