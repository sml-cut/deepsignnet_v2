...
from signjoey.transformer_layers import TransformerEncoderLayer, PositionalEncoding,TransformerSelfEncoderLayer
from signjoey.layers import LWTAMASK,LWTAMASK2,DenseBayesian,SuperMask,RandomMask
...
class TransformerEncoderEnsemble(Encoder):
    def __init__(self,*args, **kwargs):
        super(TransformerEncoderEnsemble, self).__init__()
        N=10
        self.Encoders=[TransformerEncoder_(*args, **kwargs) for i in range(N) ]
...
class TransformerEncoder(Encoder):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if framedrop=='lwta':
            ...
            self.lwta=LWTAMASK(512,4)
        elif framedrop=='superlwta':
            self.lwta=SuperMask(512,4)
        elif framedrop=='r2':
            self.lwta=RandomMask(512,2)
        elif framedrop=='r4':
            self.lwta=RandomMask(512,4)
    # pylint: disable=arguments-differ
    def forward(
        self, embed_src: Tensor, src_length: Tensor, mask: Tensor
    ) -> (Tensor, Tensor):
        if self.training:
            ...
            for i in range(2):
               ...
               x_, _=  self.forward_(embed_src,src_length,mask)
             
               x.append(torch.unsqueeze(x_,-1))
        x=torch.cat(x,-1)
    ...
    def forward_(
        ...
        if (self.framedrop=='lwta') or (self.framedrop=='superlwta') or (self.framedrop=='r2') or (self.framedrop=='r4'):
            x=self.lwta(x,x_raw)
       

       
        #if self.training: 
        #    b=torch.normal(x*0,0.01)
            
        #x= x*m+b
        #a,b=m,(1-m)
        #a,b=a/(a+b),b/(a+b)
     #   if self.training: 
         #   x=x*a+(b)*torch.normal(x*0,0.1)
          #  a=torch.zeros(x.shape).to('cuda')
            
         #   a=torch.normal(a,0.1)
