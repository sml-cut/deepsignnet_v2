...
def beam_search_(
    ...
    O=[[] for i in range(len(encoder_output))]
    for  i in range(20):
       o,_=beam_search_( decoder ,size,bos_index,eos_index,pad_index,
                    encoder_output,encoder_hidden,
                    src_mask,max_output_length,alpha,
                    embed,n_best)
    ...
    return pd.DataFrame(O).fillna(1).astype('int32').values,_

def f(x):
     a = pd.DataFrame(x).fillna(1)
     b = stats.mode(a.values)[0][0]
