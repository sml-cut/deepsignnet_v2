...
class TransformerDecoderEnsemble(Decoder):
    def __init__(self,*args, **kwargs):
        super(TransformerDecoderEnsemble, self).__init__()
        N=10
        self.Decoders=[TransformerDecoder_(*args, **kwargs) for i in range(N) ]
    ...
    def      forward(self,*args, **kwargs):
        ...
        return self.Decoders[1](*args, **kwargs)
    def forward(
        self,
        trg_embed  = None,
        encoder_output = None,
        ...
        **kwargs
        ...
        for i in range(N):
            ...
            output0, x0, _, _=self.Decoders[i](trg_embed[i],encoder_output[:,:,:,(i*2):(2+i*2)],
                                                 encoder_hidden,src_mask,unroll_steps,hidden,trg_mask,**kwargs)
            if i in [2,3]:
                output0, x0=output0*1.5, x0*1.5
            x=x+x0
            out=out+output0
        output, x= out*0.0909,x*0.0909
...
class TransformerDecoder(Decoder):
    ...
    def __init__(
       ...
       # self.bay= DenseBayesian(input_features=hidden_size, output_features=hidden_size,
            #                 competitors = 1, activation = 'linear',prior_mean=0, prior_scale=1. , ibp = False )
        ...
        if bayesian_output:
            self.output_layer = DenseBayesian(input_features=hidden_size, output_features=vocab_size,
                             competitors = 1, activation = 'linear',prior_mean=0, prior_scale=1. , kl_w=0.01,ibp = ibp)
        else:
            self.output_layer = nn.Linear(hidden_size, vocab_size, bias=False)
        ...
        **kwargs
        ...
        if self.training:
            ...
            for i in range(2):
               ...
               output_, x_, _,_ =  self.forward_(trg_embed,encoder_output[:,:,:,i],encoder_hidden,src_mask,unroll_steps,hidden,trg_mask,**kwargs)
    ...
    def forward_(
        ...
        **kwargs
        ...
        #x = F.softsign(self.bay(trg_embed))
        x=trg_embed
